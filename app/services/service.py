...
class LawyerService:
    ...
    async def get_lawyer_requests_by_status(
        self, session: AsyncSession, user_id: int, status: LawyerRequestStatusEnum
    ) -> List[LawyerRequest]:
        """
        ...
        query = select(LawyerRequest).where(
            LawyerRequest.lawyer_id == lawyer.id, LawyerRequest.status == status
        )
    ...
    async def get_document(
        ...
        if lawyer_request_id:
            ...
            if message.user_id != user_id:
                raise HTTPException(
                    status_code=403,
                    detail="Access denied. This message does not belong to you.",
                )
            ...
            raise HTTPException(
                status_code=400,
                detail="Either lawyer_request_id or message_id must be provided",
            )
