...
from lawly_db.db_models import LawyerRequest, Message, Lawyer
...
class LawyerService:
    ...
    async def create_lawyer_request_from_user(
        ...
        # TODO: добавить списание консультаций
        ...
        if document_bytes:
            ...
            encrypted_bytes = await self.gost_cipher.async_encrypt_data(document_bytes_obj, key)
    ...
    async def update_lawyer_request(
        ...
        if request.lawyer_id != lawyer.id and status == LawyerRequestStatusEnum.COMPLETED:
    ...
    async def get_document(
        ...
        if not lawyer_request_id and not message_id:
            raise ParameterError("Необходимо указать либо lawyer_request_id, либо message_id")
        
        document_url = None
        
        if lawyer_request_id:
            request = await self.lawyer_request_repo.get_lawyer_request_by_id(lawyer_request_id)
            if not request:
                raise NotFoundError(f"Заявка с ID {lawyer_request_id} не найдена")
            
            lawyer = await self.get_lawyer_by_user_id(user_id)
            
            if request.lawyer_id != lawyer.id and request.status == LawyerRequestStatusEnum.PROCESSING:
