...
class LawyerRequestRepository(BaseRepository):
    ...
    async def get_user_lawyer_request(
        ...
        query = select(LawyerRequest).where(
            LawyerRequest.user_id == user_id, LawyerRequest.id == request_id
        )
    ...
    async def get_lawyer_requests_by_status(
        ...
        """
        query = select(LawyerRequest).where(LawyerRequest.status == status)
        ...
        count_query = (
            select(func.count())
            .select_from(LawyerRequest)
            .where(LawyerRequest.status == status)
        )
        if status == LawyerRequestStatusEnum.COMPLETED:
