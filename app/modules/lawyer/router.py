...
from services.errors import AccessDeniedError, NotFoundError, ParameterError, ServiceError
...
async def get_lawyer_requests(
    status: LawyerRequestStatus = Query(..., description="Статус заявок для фильтрации"),
    current_user: JWTHeader = Depends(JWTBearer()),
    lawyer_service: LawyerService = Depends()
):
    """
    ...
    try:
        ...
        requests, total = await lawyer_service.get_lawyer_requests_by_status(
            user_id=current_user.user_id,
            status=LawyerRequestStatusEnum(status.value)
        )
        ...
        for request in requests:
            response_requests.append(
                LawyerRequestDTO(
                    ...
                    description=request.note or f"Проверка документов для заявки #{request.id}",
