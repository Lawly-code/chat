    ...
    try:
        ...
        while True:
            ...
            if data.get("type") == "user_message":
                ...
                logger.info(
                    f"Обработка сообщения от пользователя {user_id}: {message_text[:50]}..."
                )
                ...
                try:
                    ...
                    user_message = await message_repo.create_user_ai_message(
                        user_id=user_id, content=message_text
                    )
                    ...
                    await rabbitmq_service.send_ai_request(
                        user_id=user_id, message=message_text, message_id=message_id
                    )
                    logger.info(
                        f"Сообщение отправлено в RabbitMQ, message_id: {message_id}"
                    )
        ...
        logger.error(
            f"WebSocketDisconnect: код {getattr(e, 'code', 'unknown')}, причина: '{getattr(e, 'reason', '')}'"
        )
    except Exception as e:
